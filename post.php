...
if (isset($_POST['publish'])) {
    ...
    if (isset($_FILES['image'])) {
        ...
        if (move_uploaded_file($file['tmp_name'], $fileName)) {
            $query = "INSERT INTO destinations SET Name=?, Description=?, ImageURL=?, MaximumDays=?, Location=?, Lat=?, Lon=?, Elevation=?, TrekkingDifficulty=?";
            $stmt = $conn->prepare($query);

            // Adjust the types in bind_param based on the actual data types of maxdays and difficulty
            $stmt->bind_param('sssssssss', $title, $body, $fileName, $maxdays, $location, $lat, $lon, $elevation, $difficulty);
